...
//    -10^4 <= target <= 10^4
    ...
    public int[] searchRange(int[] nums, int target) {
        ...
        int index = -1;
        if (length < 286) {
            index = findIndex(0, length, nums, target);
        } else {
            index = round(0, length, nums, target);
        }
        if (index == -1) return new int[]{index, index};
        int left = index;
        int right = index;
        while (left - 1 > -1 && nums[left - 1] == target) left--;
        while (right + 1 < length && nums[right + 1] == target) right++;
    ...
    private int round(int start, int end, int[] nums, int target) {
        if (start - end <= 286) {
            ...
            if (target < nums[mid]) return round(start + 1, mid - 1, nums, target);
